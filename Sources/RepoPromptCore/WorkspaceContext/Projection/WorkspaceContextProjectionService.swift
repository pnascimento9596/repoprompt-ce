...
package struct WorkspaceContextProjectionService {
    ...
    private struct PreparedOccurrence {
        let value: WorkspaceContextProjectionMaterializationRequest.Occurrence
        let fileAPI: FileAPI?
    }
    ...
    package func project(
        ...
        let validated = try Self.validateCapture(capture)
        let preparedOccurrences = Self.makeOccurrences(
            capture: capture,
        ...
        if request.sections.contains(.selection) || request.sections.contains(.tokens) {
            ...
            selectionProjection = try WorkspaceSelectionProjectionService.project(.init(
                ...
                missingPaths: projectionMissingPaths,
                invalidPaths: projectionInvalidPaths,
    ...
    private static func makeOccurrences(
        ...
        func append(_ file: WorkspaceFileRecord, ranges: [LineRange], forceCodemap: Bool) {
            ...
            prepared.append(PreparedOccurrence(value: occurrence, fileAPI: fileAPI))
        }
        ...
        case .complete:
            for codemap in capture.codemapSnapshots where codemap.fileAPI != nil && !selectedFileIDs.contains(codemap.fileID) {
                guard let file = capture.materializedFiles.first(where: { $0.id == codemap.fileID }) else { continue }
                append(file, ranges: [], forceCodemap: true)
            }
        }
