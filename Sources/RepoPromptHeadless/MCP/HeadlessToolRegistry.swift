...
final class HeadlessToolRegistry {
    ...
    private let safeToolNames: [String] = [
        "bind_context",
        "manage_workspaces",
        "manage_selection",
        "workspace_context",
    ...
    private let gatedToolNames: Set<String> = [
        "file_actions",
        "apply_edits",
        "git",
        "manage_worktree",
    ...
    init(host: HeadlessHost) {
        self.host = host
    }

    func listDescriptors() -> [HeadlessJSONObject] {
        safeToolNames.map(descriptor(for:)).map(\.json)
    }

    func call(name: String, arguments: HeadlessJSONObject) async -> HeadlessJSONObject {
        do {
            switch name {
            case "bind_context":
            ...
            default:
                if gatedToolNames.contains(name) {
                    return HeadlessToolResponse.error("Tool '\(name)' is not available in RepoPrompt Headless v1. The standalone safe profile is read-oriented and disables app-only, write, VCS, oracle, context-builder, agent, and settings tools by policy.")
                }
            ...
            return HeadlessToolResponse.error(error.localizedDescription)
        }
    }
